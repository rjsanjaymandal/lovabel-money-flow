...
import { GameState, UnoCard, Player } from "@/types/uno";
import {
  createDeck,
  isValidMove,
  getNextPlayerIndex,
...
export default function UnoBotPage() {
  ...
  const playBotTurn = () => {
    ...
    if (validCardIndex !== -1) {
      ...
      if (bot.hand.length === 2) {
        ...
        toast({
          title: "Bot says: UNO!",
          description: "🤖 The bot is about to win!",
        });
      }
    } else {
      // Draw Card
  ...
  const handlePlayCard = (card: UnoCard, playerId: string) => {
    ...
    if (card.type === "skip") {
      skipTurn = true;
      toast({
        title: playerId === "bot" ? "Bot Skipped You!" : "You Skipped Bot!",
      });
    }
    if (card.type === "draw2") {
    ...
    if (newCard) {
      ...
      if (activeId === "bot") {
        ...
        if (isValidMove(newCard, topCard)) {
          ...
          nextState.currentPlayerIndex = getNextPlayerIndex(
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black/90 text-white p-4">
        <div className="text-center space-y-6 animate-in zoom-in">
          ...
          <Button size="lg" onClick={startLocalGame} className="text-xl">
            Play Again
          </Button>
          <Button
            variant="outline"
