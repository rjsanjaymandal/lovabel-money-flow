...
export const UnoRoomCard = ({ roomId, code, playerCount, maxPlayers = 4, hostName = "Unknown", onJoin }: UnoRoomCardProps) => {
    const isFull = playerCount >= maxPlayers;
    
    return (
        <div className="group relative bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4 hover:bg-white/5 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-primary/10 hover:border-white/20">
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-white leading-tight">{hostName}'s Room</h3>
                        <p className="text-xs text-white/50 font-mono tracking-wider">CODE: {code}</p>
                    </div>
                </div>
                <div className={cn(
                    "px-2 py-1 rounded-md text-xs font-bold border",
                    isFull ? "bg-red-500/20 text-red-500 border-red-500/50" : "bg-green-500/20 text-green-500 border-green-500/50"
                )}>
                    {isFull ? "FULL" : "OPEN"}
                </div>
            </div>
