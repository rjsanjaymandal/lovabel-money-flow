...
import { Player, GameSettings } from "@/types/uno";
...
export const UnoLobby = ({
  onCreateRoom,
  onJoinRoom,
  isLoading,
}: UnoLobbyProps) => {
  const [mode, setMode] = useState<"menu" | "create" | "join" | "browse">(
    "menu",
  );
  ...
  const fetchRooms = async () => {
    ...
    if (states) {
      const mapped: RoomData[] = (states as unknown[]).map((state) => {
        const s = state as {
          id: string;
          room_id: string;
          players: unknown;
        ...
        return {
          id: s.id,
          room_id: s.room_id,
          players: (s.players as unknown as Player[]) || [],
          code: s.uno_rooms.code,
          status: s.uno_rooms.status as "waiting" | "playing" | "finished",
          settings: s.uno_rooms.settings as GameSettings,
        };
      });

      setActiveRooms(
        mapped.filter(
          (s) => s.status === "waiting" && s.settings?.isPublic !== false,
        ),
      );
    }
    setIsFetching(false);
  ...
  const fetchMyRooms = async () => {
    ...
    if (myRoomsData) {
      const mapped: RoomData[] = (myRoomsData as unknown[]).map((room) => {
        const r = room as {
          id: string;
          code: string;
          status: string;
          settings: unknown;
          uno_game_states: Array<{ players: unknown }>;
        };
        return {
          id: r.id,
          room_id: r.id,
          players:
            (r.uno_game_states?.[0]?.players as unknown as Player[]) || [],
          code: r.code,
          status: r.status as "waiting" | "playing" | "finished",
          settings: r.settings as GameSettings,
        };
      });
      setMyRooms(mapped);
    }
      ...
      >
        {mode !== "create" && (
          <Button
            ...
            onClick={() =>
              mode === "menu" ? navigate("/dashboard") : setMode("menu")
            }
          >
              ...
              <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-primary font-bold text-xl px-3 py-1 bg-primary/10 rounded-lg">
                          {startingCards[0]}
                        </span>
                      </div>
                      <Slider
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onCreateRoom(startingCards[0], isPublic, maxPlayers[0])
                    }
                    disabled={isLoading}
                  >
                    {isLoading ? (
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="text-xs text-white/40">
                              {room.players.length} Players • {room.status}
                            </span>
                          </div>
