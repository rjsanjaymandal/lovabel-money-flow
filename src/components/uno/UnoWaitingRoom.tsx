          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {gameState.players.map((player, index) => (
                <div
                  key={player.id}
                  className="group relative flex flex-col items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all animate-in fade-in slide-in-from-bottom-4"
              ...
              {/* Empty Slots */}
              {Array.from({
                length: Math.max(0, maxPlayers - gameState.players.length),
              }).map((_, i) => (
                <div
                  key={`empty-${i}`}
                  className="flex flex-col items-center gap-3 p-4 rounded-xl border-2 border-dashed border-white/5 opacity-50"
                >
