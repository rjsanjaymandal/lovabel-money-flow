...
import { GameState, UnoCard as UnoCardType, Player } from "@/types/uno";
    ...
    <div className="w-full h-[100dvh] bg-background text-foreground flex flex-col overflow-hidden relative">
        ...
        {gameState.winner && (
          ...
          >
            <div className="text-center space-y-8 animate-in zoom-in duration-500">
              ...
              <Button
                size="lg"
                className="text-2xl px-12 py-8 rounded-full font-black animate-pulse"
                onClick={onExit}
              >
                Back to Lobby
              </Button>
            ...
            <div className="bg-zinc-900 p-8 rounded-3xl border border-white/10 shadow-2xl text-center space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="w-24 h-24 bg-red-500 rounded-2xl hover:scale-110 transition-transform shadow-lg shadow-red-500/30"
                />
                <button
                  onClick={() => handleColorSelect("blue")}
                  className="w-24 h-24 bg-blue-500 rounded-2xl hover:scale-110 transition-transform shadow-lg shadow-blue-500/30"
                />
                <button
                  onClick={() => handleColorSelect("green")}
                  className="w-24 h-24 bg-emerald-500 rounded-2xl hover:scale-110 transition-transform shadow-lg shadow-emerald-500/30"
                />
                <button
                  onClick={() => handleColorSelect("yellow")}
                  className="w-24 h-24 bg-yellow-400 rounded-2xl hover:scale-110 transition-transform shadow-lg shadow-yellow-500/30"
                />
              </div>
              <Button
                variant="ghost"
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-2">
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button
                    size="icon"
                    variant="ghost"
                    className="rounded-full bg-black/40 text-white hover:bg-red-500/20 hover:text-red-500 backdrop-blur-md transition-colors"
                  >
                ...
                <AlertDialogContent className="bg-zinc-900 border-white/10 text-white">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-white/60">
                      Are you sure you want to leave? You won't be able to
                      rejoin easily if the game is full.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
            ...
            {!hideRoomCode && (
              ...
              >
                <div>
                  <span className="text-[10px] font-bold text-white/60 uppercase tracking-wider block leading-none mb-0.5">
                    Room Code
                  </span>
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-white/10 ring-2 ring-primary shadow-lg scale-105 z-10"
                    : "bg-black/20 border border-white/5 opacity-70 scale-95",
                )}
              >
                ...
                <span className="text-[10px] font-bold text-white truncate max-w-[80px] mb-1">
                  {player.name}
                </span>
                    ...
                    (_, i) => (
                      <motion.div
                        ...
                        animate={{
                          scale: 1,
                          rotate:
                            (i - Math.min(player.hand.length, 10) / 2) * 10,
                          x: (i - Math.min(player.hand.length, 10) / 2) * 2,
                        }}
      ...
      <div className="flex-1 relative flex items-center justify-center min-h-0 perspective-[1000px]">
        <div className="flex items-center gap-8 sm:gap-24 pl-4 sm:pl-0">
          ...
          <motion.div
            ...
            whileTap={
              isMyTurn && !isDrawing && !gameState.hasDrawnThisTurn
                ? { scale: 0.95 }
                : {}
            }
            onClick={
          ...
          >
            ...
            <span className="font-black text-white/20 text-3xl -rotate-45">
              UNO
            </span>
            ...
            {(showForcedDraw || isDrawing) && (
              <div
                className={cn(
                  ...
                  isDrawing
                    ? "ring-yellow-500/50 bg-black/20"
                    : "ring-red-500/50 animate-pulse",
                )}
              >
                <span
                  className={cn(
            ...
            <div
              className={cn(
                ...
                gameState.direction === 1
                  ? "animate-spin-slow border-t-primary/80"
                  : "animate-reverse-spin border-t-purple-500/80",
              )}
            />
              ...
              >
                {topCard ? (
                  <UnoCard
                    card={topCard}
                    size="lg"
                    className="shadow-2xl w-24 h-32 sm:w-36 sm:h-52"
                  />
                ) : (
      ...
      >
        ...
        <div className="flex justify-center items-center gap-4 mb-2 min-h-[60px] px-4">
          {/* UNO Button */}
          {me?.hand.length === 1 &&
            !gameState.players.find((p) => p.id === me.id)?.isUno && (
              <Button
                variant="destructive"
                size="lg"
