...
export function NewsView() {
  ...
  const analyzeNews = (item: NewsItem) => {
    ...
    if (
      text.match(
        /surge|soar|record|high|profit|jump|gain|bull|rally|growth|positive/,
      )
    )
      sentiment = "bullish";
    if (
      text.match(
        /crash|plunge|drop|loss|bear|fall|down|negative|warn|risk|crisis/,
      )
    )
      sentiment = "bearish";
    ...
    else if (text.match(/ai|tech|apple|google|microsoft|nvidia|software/))
      sector = "Tech";
  ...
  const fetchNews = async () => {
    ...
    try {
        ...
        RSS_FEEDS.map(async (feed) => {
          try {
            ...
            if (data.status === "ok") {
              ...
              const items: NewsItem[] = (data.items as RSSItem[]).map(
                (item) => {
                  const { sentiment, sector } = analyzeNews(
      ...
      const uniqueNews = sortedNews.filter(
        (item, index, self) =>
          index ===
          self.findIndex((t) => t.title === item.title || t.guid === item.guid),
      );
  ...
  const handleCardClick = (item: NewsItem) => {
    ...
    if (item.sector && item.sector !== "General") {
      const newInterests = {
        ...userInterests,
        [item.sector]: (userInterests[item.sector] || 0) + 1,
      };
  ...
  const generateLocalHighlights = (htmlContent: string) => {
    ...
    setTimeout(() => {
      ...
      const scoredSentences = sentences.map((sentence) => {
        ...
        if (lower.match(/surge|plunge|soar|crash|record|high|low|profit|loss/))
          score += 3;
      ...
      setHighlights(
        topSentences.length > 0 ? topSentences : ["No key highlights found."],
      );
      setIsGenerating(false);
    }, 800);
  };
  ...
  const filteredNews = news.filter((item) => {
    ...
    const matchesCategory =
      selectedCategory === "All" || item.sector === selectedCategory;
  ...
  const relatedNews = selectedNews
    ? news
        .filter(
          (n) =>
            n.sector === selectedNews.sector && n.guid !== selectedNews.guid,
        )
        .slice(0, 3)
    : [];
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-background gap-6 p-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-xs mx-auto">
            We couldn't connect to the financial feeds. Please check your
            connection.
          </p>
        </div>
    ...
    <div className="min-h-screen bg-background relative pb-24 sm:pb-10">
      ...
      <div className="container max-w-7xl mx-auto px-0 sm:px-4 pt-4 sm:pt-6">
        <div className="border-b border-border/50 sm:border-0 pb-4 sm:pb-2">
          <h2 className="px-4 sm:px-0 text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">
            Market Pulse
          </h2>
          <MarketStories />
        </div>
            ...
            >
              ...
              <div className="flex-1 h-full overflow-y-auto pb-safe overscroll-y-contain">
                ...
                <div className="relative w-full h-[35vh] sm:h-[350px]">
                  <motion.div
                    ...
                    style={{
                      backgroundImage: `url(${getImageUrl(selectedNews)})`,
                    }}
                  />
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      ...
                      {selectedNews.sentiment &&
                        selectedNews.sentiment !== "neutral" && (
                          <div
                            className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 border backdrop-blur-md ${
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground font-medium bg-background/50 px-2 py-1 rounded-md backdrop-blur-sm">
                        ...
                        {formatDistanceToNow(new Date(selectedNews.pubDate), {
                          addSuffix: true,
                        })}
                      </div>
                    </div>
                ...
                <div className="p-6 pt-0 space-y-6 pb-32">
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                    ...
                    <Button
                      ...
                      className={`flex-1 gap-2 min-w-[50px] ${
                        bookmarks.includes(selectedNews.guid)
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html:
                        selectedNews.description || selectedNews.contentSnippet,
                    }}
                  />
                    ...
                    <div className="p-4 border-b border-border/50 bg-black/20 backdrop-blur-sm flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          Ask about impacts, sectors, or trends
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 space-y-4 max-h-[300px] overflow-y-auto">
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="bg-card border border-border/50 p-3 rounded-2xl rounded-tl-none text-sm leading-relaxed shadow-sm">
                          <p>
                            I've analyzed this article. Ask me anything! e.g.,
                            "Is this good for Banking stocks?"
                          </p>
                        </div>
                      </div>
                      ...
                      {chatHistory.map((msg, i) => (
                        ...
                        >
                          ...
                          <div
                            className={`p-3 rounded-2xl text-sm leading-relaxed shadow-sm max-w-[80%] ${
                              msg.role === "user"
                        ...
                        onKeyDown={async (e) => {
                          if (e.key === "Enter" && !isAnalyzing) {
                            ...
                            setTimeout(() => {
                              ...
                              const response = `Based on current market sentiment (${
                                selectedNews.sentiment
                              }), this indicates a potential ${
                                selectedNews.sentiment === "bullish"
                                  ? "uptrend"
                                  : "correction"
                              } for ${selectedNews.sector || "the market"}. ${
                                question.toLowerCase().includes("time")
                                  ? "It might be a good time to watch."
                  ...
                  {relatedNews.length > 0 && (
                    <div className="pt-8 border-t border-border/50">
                      <h3 className="text-lg font-bold mb-4">
                        Related in {selectedNews.sector}
                      </h3>
              ...
              >
                <div className="space-y-8">
                  ...
                  <div
                    className="w-full h-[600px] rounded-3xl bg-cover bg-center shadow-2xl"
                    style={{
                      backgroundImage: `url(${getImageUrl(selectedNews)})`,
                    }}
                  />
                  ...
                  <div className="space-y-6">
                    <div className="flex gap-4">
                      ...
                      {selectedNews.sentiment && (
                        <span
                          className={`px-6 py-2 rounded-full text-2xl font-bold uppercase tracking-wider ${
                            selectedNews.sentiment === "bullish"
                ...
                <div className="space-y-8">
                    ...
                    <ul className="space-y-4">
                      {(highlights.length > 0
                        ...
                        .map((point, i) => (
                          <li
                            key={i}
                            className="text-3xl text-muted-foreground leading-relaxed"
                          >
                            • {point}
                          </li>
                        ))}
                  ...
                  <p className="text-center text-2xl text-muted-foreground">
                    Read more on Lovabel Money Flow
                  </p>
                </div>
              </div>
            </motion.div>
