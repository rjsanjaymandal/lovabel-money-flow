  ...
  if (loading) {
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-muted-foreground/50 text-sm font-medium">Loading transactions...</p>
      </div>
    );
  }
  ...
  if (transactions.length === 0) {
    return (
      <div className="text-center py-16 px-4 rounded-3xl bg-white/5 border border-white/5 backdrop-blur-sm">
        ...
        <h3 className="text-lg font-medium text-foreground/80 mb-1">No transactions yet</h3>
        <p className="text-muted-foreground/50 text-sm">Add your first expense to get started.</p>
      </div>
    );
  }
    ...
    <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
      <AlertDialogContent className="rounded-3xl border-white/10 bg-black/40 backdrop-blur-xl">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={handleDelete} className="bg-rose-500 hover:bg-rose-600 text-white rounded-xl">
            Delete
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
