...
import { Sparkles, Send, Bot, User, Trash2, PieChart as PieChartIcon, ArrowUpRight, ArrowDownRight, X } from "lucide-react";
...
export const AIInsights = ({ userId, selectedMonth, income, expenses, transactions, budget = 0, userName = "Friend" }: AIInsightsProps) => {
  ...
  const analyzeRequest = (query: string): Partial<Message> => {
    ...
    try {
      if (lowerQuery.includes("breakdown") || lowerQuery.includes("chart") || lowerQuery.includes("graph") || lowerQuery.includes("where") && lowerQuery.includes("go")) {
        ...
        const chartData = Object.entries(categoryTotals).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 5);
        if (chartData.length > 0) return { content: `Spending breakdown for ${timeContext}. Top: **${chartData[0].name}**.`, type: "chart", data: chartData };
      ...
      if (lowerQuery.includes("highest") || lowerQuery.includes("biggest") || lowerQuery.includes("top expense")) {
        const topExpenses = relevantTransactions.filter(t => t.type === "expense").sort((a, b) => Number(b.amount) - Number(a.amount)).slice(0, 3);
        if (topExpenses.length > 0) return { content: `Top expenses for ${timeContext}:`, type: "list", data: topExpenses };
      ...
      if (lowerQuery.includes("show") || lowerQuery.includes("list") || lowerQuery.includes("find")) {
        ...
        if (foundCategory) {
           const categoryTxns = relevantTransactions.filter(t => t.category === foundCategory).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5);
           if (categoryTxns.length > 0) return { content: `Found ${categoryTxns.length} recent **${foundCategory}** transactions:`, type: "list", data: categoryTxns };
        }
      }
      if (lowerQuery.includes("budget")) {
        if (budget === 0) return { content: "No budget set. Go to the Budget tab! 🎯" };
        const remaining = budget - expenses, percent = (expenses / budget) * 100;
        return {
           content: remaining > 0 ? `✅ **₹${remaining.toLocaleString()}** left (${(100-percent).toFixed(1)}%).` : `🚨 Over by **₹${Math.abs(remaining).toLocaleString()}**!`,
           type: "insight", data: { remaining, budget, expenses, percent }
        };
      }
      return { content: getRandomResponse(["Try asking: 'Spending breakdown', 'Top expenses', or 'Budget status'."]) };
    } catch (err) { console.error(err); return { content: "Error processing request." }; }
  };

  const handleSendMessage = async (text: string) => {
    if (!text.trim()) return;
    setMessages(prev => [...prev, { role: "user", content: text, type: "text", timestamp: new Date() }]);
    setInputValue("");
    setIsTyping(true);
    const delay = Math.floor(Math.random() * 500) + 600;
    setTimeout(() => {
      const response = analyzeRequest(text);
      setMessages(prev => [...prev, { role: "ai", content: response.content || "Error", type: response.type || "text", data: response.data, timestamp: new Date() }]);
      setIsTyping(false);
    }, delay);
  };
  ...
  const renderMessageContent = (msg: Message) => (
    <div className="space-y-3 w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
      <p className="whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{ __html: msg.content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br/>') }} />
      {msg.type === "chart" && msg.data && (
        <div className="h-40 w-full bg-black/20 rounded-xl border border-white/5 p-2 mt-2 backdrop-blur-md shadow-inner">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={msg.data} cx="50%" cy="50%" innerRadius={25} outerRadius={50} paddingAngle={4} dataKey="value" stroke="none">
                {msg.data.map((entry: any, index: number) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
              </Pie>
              <Tooltip contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: 'none', borderRadius: '8px', fontSize: '11px' }} itemStyle={{ color: '#fff' }} />
      ...
      {msg.type === "list" && msg.data && (
        <div className="space-y-1.5 mt-2">
          {msg.data.map((txn: any, i: number) => (
            <div key={i} className="flex items-center justify-between p-2 bg-white/5 border border-white/5 rounded-lg text-xs hover:bg-white/10 transition-colors">
               <div className="flex items-center gap-2">
                 <div className={`w-1.5 h-1.5 rounded-full ${txn.type === 'income' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                 <span className="truncate max-w-[90px] text-muted-foreground">{txn.description || txn.category}</span>
               </div>
               <span className={`font-medium ${txn.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
    ...
    <>
      <Card className="border-border/50 shadow-lg animate-scale-in bg-gradient-to-br from-indigo-500/5 to-purple-500/5 relative z-50 overflow-hidden">
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={() => setIsOpen(true)} className="w-full gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg shadow-indigo-500/20 cursor-pointer relative z-50 transition-all hover:scale-[1.02]">
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        <SheetContent side="bottom" className="h-[92vh] sm:h-[85vh] sm:max-w-lg sm:mx-auto sm:rounded-t-[32px] p-0 flex flex-col bg-background/95 backdrop-blur-2xl border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.2)]">
          {/* Header */}
            ...
            <div className="space-y-6 pb-4">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`rounded-2xl px-4 py-3 max-w-[85%] text-sm shadow-sm transition-all duration-300 ${
                    msg.role === "user" 
                    ...
                    <div className={`text-[9px] mt-1.5 opacity-60 font-medium ${msg.role === "user" ? "text-indigo-100" : "text-muted-foreground"}`}>
          ...
          <div className="p-4 sm:p-5 pb-safe pt-2 bg-gradient-to-t from-background via-background to-transparent z-10 shrink-0">
            ...
            <div className="flex items-end gap-2 p-1.5 bg-muted/40 border border-white/10 rounded-[28px] shadow-lg backdrop-blur-xl transition-all focus-within:ring-1 focus-within:ring-indigo-500/30 focus-within:bg-muted/60">
              ...
              <Button 
                ...
                size="icon" 
                className={`h-9 w-9 rounded-full shrink-0 transition-all duration-300 ${inputValue.trim() || isTyping ? 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-500/25' : 'bg-muted text-muted-foreground opacity-50 cursor-not-allowed'}`}
